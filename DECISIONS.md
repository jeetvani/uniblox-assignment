...
The supplied mock API runs independently on port 4000. It is downloaded from the assignment repository for local use, remains unchanged, and is not tracked in this submission. Browser requests use relative `/api` paths, and Vite proxies them to the configurable `API_PROXY_TARGET` during development. Search, employer group, review reason, and supported sorts use the documented API contract. Extended coverage, product, priority, date, and completeness filters are derived locally from returned queue data.
