...
function sortRecords(items, sort) {
  ...
  return [...items].sort((left, right) => {
    ...
    switch (sort) {
      ...
      case 'coverage_desc':
        comparison = compareNullable(
          Number.isFinite(left.coverageAmountCents) ? left.coverageAmountCents : null,
          Number.isFinite(right.coverageAmountCents) ? right.coverageAmountCents : null,
          -1,
        );
        break;
      case 'coverage_asc':
...
function listSubmissions(url) {
  ...
  const product = normalize(url.searchParams.get('product'));
  const priority = normalize(url.searchParams.get('priority'));
  const submitted = normalize(url.searchParams.get('submitted'));
  const completeness = normalize(url.searchParams.get('completeness'));
  const coverageMin = Number(url.searchParams.get('coverageMin'));
  const coverageMaxValue = url.searchParams.get('coverageMax');
  const coverageMax = coverageMaxValue === null ? null : Number(coverageMaxValue);
  ...
  if (product) {
    matches = matches.filter(
      (record) => normalize(record.product) === product,
    );
  }

  if (priority) {
    matches = matches.filter(
      (record) => normalize(record.priority) === priority,
    );
  }

  if (submitted === 'with_date') {
    matches = matches.filter((record) => Boolean(record.submittedAt));
  } else if (submitted === 'missing_date') {
    matches = matches.filter((record) => !record.submittedAt);
  }

  if (completeness) {
    const hasMissingQueueData = (record) => [
      record.applicant?.name,
      record.applicant?.email,
      record.group?.id,
      record.product,
      record.coverageAmountCents,
      record.submittedAt,
      record.reviewReason,
      record.status,
  ...
  if (Number.isFinite(coverageMin) && coverageMin > 0) {
    matches = matches.filter(
      (record) => Number.isFinite(record.coverageAmountCents) &&
        record.coverageAmountCents >= coverageMin * 100,
    );
  }

  if (Number.isFinite(coverageMax)) {
    matches = matches.filter(
      (record) => Number.isFinite(record.coverageAmountCents) &&
        record.coverageAmountCents <= coverageMax * 100,
    );
  }
